...
//            }
        ...
        if (!empty($this->filterFields)) {
            foreach ($this->filterFields as $key => $field) {
                ...
                } else if (isset($_GET[$field]) || isset($_GET[$key])) { // check với trường hợp rewrite URL, chỉ nhận $_GET ko nhận $_REQUEST
                    if (is_numeric($key)) { // nếu ko fai là trường rewrite (gender => sex )
        ...
        // filters
        $tmpFilters = Yii::$app->request->getQueryParam('filters', []);
